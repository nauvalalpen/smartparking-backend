<x-app-layout>
    ...
    <div x-data="{
        ...
        @endif
        @if ($errors->any())
            <div class="toast toast-error" x-data x-init="@if (old('_form') === 'edit') showEdit = true; edit = { id: '{{ old('_id') }}', id_area: '{{ old('id_area') }}', nama_kamera: '{{ old('nama_kamera') }}', rtsp_url: '{{ old('rtsp_url') }}', status: '{{ old('status', 'aktif') }}' };
                    ...
                    <table class="data-table">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align:center;">Test AI</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="cam-name-cell">
                                            <div
                                                class="cam-dot {{ $kamera->status === 'aktif' ? 'cam-dot-on' : 'cam-dot-off' }}">
                                            </div>
                                    ...
                                    <td>
                                        <div class="action-row">
                                            ...
                                            {{-- TOMBOL ACTION UNTUK TESTING AI --}}
                                    <td class="px-4 py-2 border border-gray-200 text-center">
                                        <div class="flex items-center justify-center gap-2">

                                            {{-- TOMBOL START AI (BARU) --}}
                                            <form action="{{ route('kamera.start_ai', $kamera->id_kamera) }}"
                                                method="POST" style="display:inline;">
                                                @csrf
                                                <button type="submit"
                                                    class="bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold py-1 px-3 rounded shadow flex items-center gap-1 transition-all">
            ...
            <form action="{{ route('kamera.store') }}" method="POST" class="modal-form">
                        ...
                        <select name="id_area" class="field-input" required>
                            ...
                            @foreach ($areas as $area)
                                <option value="{{ $area->id_area }}"
                                    {{ old('id_area') == $area->id_area ? 'selected' : '' }}>{{ $area->nama_area }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="field-group">
                    ...
                    <input type="text" name="nama_kamera" class="field-input" value="{{ old('nama_kamera') }}"
                        placeholder="Contoh: CCTV Pintu Utama A" required autofocus>
    ...
    <div x-show="showEdit" class="modal-backdrop" @click.self="showEdit = false" style="display:none;"
        ...
        <div class="modal-box" x-transition:enter="t-pop" x-transition:enter-start="pop-out"
            ...
            <form :action="'{{ url('kamera') }}/' + edit.id" method="POST" class="modal-form">
                ...
                <div class="field-row-2">
                    <div class="field-group">
                        ...
                        <select name="id_area" class="field-input" required>
                            ...
                            @foreach ($areas as $area)
                                <option value="{{ $area->id_area }}"
                                    :selected="edit.id_area == '{{ $area->id_area }}'">{{ $area->nama_area }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field-group">
                ...
                <div class="field-group">
                    <label class="field-label">Nama Kamera <span class="req">*</span></label>
                    <input type="text" name="nama_kamera" class="field-input" x-model="edit.nama_kamera"
                        required>
                </div>

                <div class="field-group">
                    <label class="field-label">URL RTSP <span class="req">*</span></label>
                    <input type="url" name="rtsp_url" class="field-input field-mono" x-model="edit.rtsp_url"
                        required placeholder="rtsp://user:pass@192.168.1.100/stream">
                    <p class="field-hint">Format: <code>rtsp://username:password@ip-address/path</code></p>
                </div>
